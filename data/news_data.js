window.NEWS_DATA = {
 "date": "יום רביעי, 16 בספטמבר 2026",
 "date_he": "16/09/2026",
 "updated": "23:24",
 "lead": {
  "section": "פוליטיקה",
  "eyebrow": "הכותרת של הבוקר",
  "title": "סער המריא בחשאי לניו יורק, הסיבה: שדרוג יחסים עם מרוקו | זו המשמעות",
  "summary": "ישראל ומרוקו כבר הכריזו על חידוש היחסים הדיפלומטיים ב-2020, אך שר החוץ טס במיוחד לארה\"ב כדי לסכם על שדרוג נוסף. דרג הנציגויות יעלה לשגרירויות, הטיסות הישירות יתוגברו. על הפרק: גם הסכמים כלכליים חדשים",
  "link": "https://www.ynet.co.il/news/article/hjkzgh00tzl",
  "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/09/16/S1i00QD00Kzx/S1i00QD00Kzx_1_396_852_480_0_medium.jpg"
 },
 "articles": [
  {
   "section": "מזג אוויר",
   "title": "עתלית · 30.7° / 24.4°",
   "summary": "כרגע 26.3°, לחות 85%, רוח 7.2 קמ״ש. סיכוי משקעים 0%.",
   "source": "Open-Meteo · השירות המטאורולוגי",
   "link": "",
   "stats": [
    [
     "מקסימום",
     "30.7°"
    ],
    [
     "מינימום",
     "24.4°"
    ],
    [
     "לחות",
     "85%",
     85
    ],
    [
     "רוח",
     "7.2 קמ״ש"
    ],
    [
     "סיכוי גשם",
     "0%",
     0
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · ישראל",
   "summary": "עדכון חי ממשחקי הכדורגל בישראל - מתעדכן בכל איסוף.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football/israel",
   "image": null,
   "stats": [
    [
     "הפועל פתח תקווה נגד הפועל באר שבע",
     "0:2 · הסתיים"
    ],
    [
     "Maccabi Hertzliya נגד MS Kfar Kassem",
     "1:0 · הסתיים"
    ],
    [
     "Hapoel Akko נגד SC Ashdod",
     "0:4 · הסתיים"
    ],
    [
     "Hapoel Ramat Gan נגד מכבי נתניה",
     "2:0 · הסתיים"
    ],
    [
     "הפועל חיפה נגד בני סכנין",
     "3:1 · הסתיים"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · עולם",
   "summary": "המשחקים החשובים בעולם כרגע, מהתחרויות המוכרות ביותר.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football",
   "image": null,
   "stats": [
    [
     "AC Milan נגד Benfica",
     "0:2 · הסתיים"
    ],
    [
     "Sunderland נגד AZ Alkmaar",
     "1:0 · הסתיים"
    ],
    [
     "Sturm Graz נגד Rennes",
     "0:0 · הסתיים"
    ],
    [
     "Bayer Leverkusen נגד NK Celje",
     "2:0 · הסתיים"
    ],
    [
     "Olympiacos נגד Jagiellonia",
     "2:1 · הסתיים"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "שכחו אותו בבית: איך זה ברק יצחקי עדיין ללא קבוצה? | טור",
   "summary": "הנחת העבודה הייתה שבשלב זה של העונה ברק יצחקי כבר ימצא קבוצה חדשה, אבל בינתיים הטלפון שלו לא מצלצל",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelisoccer/article/rj9vptoffl",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/05/12/SkWSJqkZJMe/SkWSJqkZJMe_0_153_3000_1689_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "\"יש לי אלף עוקבים, זו בדיחה. לא מחפשת חשיפה ותהילה, אני ספורטאית מהלב\"",
   "summary": "בגיל 20, עם כומתה על הכתף, אבא שהוא מאמן אגדי, חברות קרובות שהפכו ליריבות על הכרטיס האולימפי היחיד, בלי אינסטגרם פעיל ובלי פחד לצאת לים גם כשטילים עפים מעליה, תמר שטיינברג מתרגלת למעמדה החדש כאלופת אירופה והעולם בגלישת רוח: \"אני מהאנשים הכי תחרותיים שתכיר. ברו…",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/article/by5alyotze",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/09/16/B1kGYSOKGx/B1kGYSOKGx_0_0_850_479_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "הדבר האמיתי: שתי התל-אביביות מגיעות לדרבי שהוא הכל חוץ ממשחק הכנה",
   "summary": "תקציבי שיא של עשרות מיליוני דולרים, מאבקי שליטה בהנהלה ומעברים טעונים של ים מדר ותמיר בלאט. מכבי והפועל ת\"א נפגשות הערב לדרבי הראשון של העונה במסגרת הסופרקאפ החברתי, רגע לפני שהדבר האמיתי ביורוליג מתחיל",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelibasketball/article/rywdftdygx",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/06/16/BJcCwfzkffl/BJcCwfzkffl_0_119_3000_1689_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "\"דברים שלקח לחיזבאללה 20 שנה לפתח, החות'ים פיתחו בחמש שנים\"",
   "summary": "הלחימה בין סעודיה לחות'ים נמשכת, ובצל הידלדלות מלאי המיירטים של ארה\"ב במלחמה, ריאד פונה לבעלות ברית אחרות בבקשה לסיוע בהגנה מפני המורדים. התנועה במצר הורמוז עדיין דלילה, הצינור העוקף נפגע והושבת, וכעת הסעודים מנסים דרך חדשה להעברת נפט גולמי. מחירי הנפט בסין הג…",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/bygqqvotml",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2025/05/24/B1W14l1zgl/B1W14l1zgl_0_0_850_479_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "מחיר מדיני בבקשה",
   "summary": "הברית הסעודית עם ארה\"ב נסדקת, ההסכם עם פקיסטן וטורקיה לא עומד במבחן ובתימן מגבירים לחץ על הממלכה. כעת נפתחת בפני ישראל הזדמנות",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/yokra14901580",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2025/11/18/BklVRdV9xZl/BklVRdV9xZl_0_251_1834_1033_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "האם ChatGPT עומד להציג פרסומות גם בישראל?",
   "summary": "לכולנו היה ברור שזה עניין של זמן, ובין אם נרצה את זה או לא, הפרסומות נמצאות בדרך לכלי ה-AI השונים. כעת, אחרי ניסוי שככל הנראה היה די מוצלח, OpenAI מתחילה להתריע בפני המשתמשים שלה שהרגע הזה מתקרב – ונראה שגם המשתמשים הישראליים בדרך לקבל פרסומות ב-ChatGPT. לא מש…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/openai-privacy-policy-update-ads/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "הסטארטאפ שמנסה להחליף מורים לאנגלית ב-AI גייס עוד 22 מיליון דולר",
   "summary": "כן כן. אנחנו יודעים שא-ת-ם מתכתבים עם הקולגה Itzik באנגלית של לורד אנגלי ושואלים אותו מתי הוא מגיע לפינת ה-Tami-Four, אבל חלק עצום של אוכלוסיית העולם לא דובר אנגלית ברמת שפת אם. הסטארטאפ Loora מפתח אפליקציית AI שעוזרת למיליוני אנשים ללמוד אנגלית, והוא מגייס עו…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/loora-raises-22m-series-b/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "חוקר בגוגל התפטר ומזהיר: \"ה-AI יכול לחסל את כולנו\"",
   "summary": "הסערות סביב תחום ה-AI לא עוצרות, ועובד באחת מהשחקניות הגדולות בתחום החליט לעזוב אותה בקול תרועה עם אזהרה חריגה – והוא לא לבד. הבעיה הכי גדולה \"לאחרונה התפטרתי מ-Google DeepMind… בתקופתי בגוגל ראיתי את פיתוח ה-AI מקרוב ובאופן ישיר. גם אני מוטרד עמוקות מהמסלול ש…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/google-ai-researcher-leaves-and-warns-of-ai-dangers/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "\"את חלק ממני\": הדואט המפתיע של שרית חדד ובתה בפארק הירקון",
   "summary": "בתה הבכורה של חדד, נויה בת התשע, עלתה לבמה בהופעת 30 שנות הקריירה של אמה בפארק הירקון  - וריגשה את הקהל כששתיהן ביצעו יחד את \"חלק ממני\", השיר שחדד הוציאה לאחר לידתה",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/bkcowuofge",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/09/16/Hkj1KuOKfx/Hkj1KuOKfx_173_1_1459_821_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "אד שירן והמופע שהפך לסיוט: איך מקלמור סיבך את הכוכב הכי סטרילי בפופ",
   "summary": "במשך יותר מעשור אד שירן בנה לעצמו תדמית נדירה של סופרסטאר שמצליח להישאר כמעט לחלוטין מחוץ לסערות ציבוריות ופוליטיות. אלא שמופע החימום של מקלמור שינה את המשוואה - בעיקר אחרי שהודח מחימומו של שירן בהמשך מסע ההופעות. שירן ניסה להבהיר שאין לו קשר להחלטה, אבל מצא א…",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/hkyf3i00kmx",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/09/16/BJK2kvdFfg/BJK2kvdFfg_0_0_850_479_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "הזוכה הראשון בתולדות The Voice נמצא מת באגם בגיל 43",
   "summary": "בן סונדרס, שזכה בעונה הראשונה של The Voice of Holland ב-2011 והיה למעשה הזוכה הראשון בתולדות הפורמט שעלה ביותר מ-150 גרסאות ברחבי העולם, נמצא מת באגם באזור ארנהם שבהולנד. גופתו נמצאה לאחר שדווח כנעדר והמשטרה פתחה בחיפושים נרחבים באזור. המשטרה מסרה כי אין חשד ל…",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/b1lqhnotzl",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/09/16/rJRt1rOKGe/rJRt1rOKGe_0_20_1149_647_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "לומדים",
   "learn": true,
   "title": "מדוע חתולים מגרגרים",
   "summary": "גרגור בתדר 25-150Hz עשוי לזרז ריפוי עצמות ולהרגיע — לא רק סימן לשמחה.",
   "source": "תמצית ידע יומית · ביולוגיה",
   "link": "",
   "stats": [
    [
     "תחום",
     "ביולוגיה"
    ],
    [
     "עובדה בונוס",
     "כוכב הנוגה מסתובב הפוך משאר כוכבי הלכת."
    ]
   ]
  }
 ]
};