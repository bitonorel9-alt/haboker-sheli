window.NEWS_DATA = {
 "date": "יום שני, 3 באוגוסט 2026",
 "date_he": "03/08/2026",
 "updated": "19:19",
 "lead": {
  "section": "פוליטיקה",
  "eyebrow": "הכותרת של הבוקר",
  "title": "המחליף המיועד של כ\"ץ לאלוף פיקוד המרכז בכלל לא התלהב מההצעה",
  "summary": "אלוף דדו בר כליפא זומן לפני כ-3 חודשים לשיחה אצל הרמטכ\"ל, ועל הפרק עמדה האפשרות שימונה במקומו של אלוף אבי בלוט. כליפא, שהבין שיחטוף אש מכל כיוון, הבהיר כי יעדיף להיות בפיקוד הצפון או הדרום, אך בסוף התרצה. גורמים: הוא העדיף להיות המזכיר הצבאי של רה\"מ עד שיתפנה …",
  "link": "https://www.ynet.co.il/news/article/hyv3v80sfe",
  "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2025/07/17/r1Dj58EL8lg/r1Dj58EL8lg_0_77_1280_720_0_medium.jpg"
 },
 "articles": [
  {
   "section": "מזג אוויר",
   "title": "עתלית · 31.9° / 27.3°",
   "summary": "כרגע 27.6°, לחות 79%, רוח 10.5 קמ״ש. סיכוי משקעים 0%.",
   "source": "Open-Meteo · השירות המטאורולוגי",
   "link": "",
   "stats": [
    [
     "מקסימום",
     "31.9°"
    ],
    [
     "מינימום",
     "27.3°"
    ],
    [
     "לחות",
     "79%",
     79
    ],
    [
     "רוח",
     "10.5 קמ״ש"
    ],
    [
     "סיכוי גשם",
     "0%",
     0
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · ישראל",
   "summary": "עדכון חי ממשחקי הכדורגל בישראל - מתעדכן בכל איסוף.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football/israel",
   "image": null,
   "stats": [
    [
     "עירוני טבריה נגד Kiryat Shmona",
     "0:1 · הסתיים"
    ],
    [
     "בני סכנין נגד הפועל חיפה",
     "1:2 · הסתיים"
    ],
    [
     "הפועל ירושלים נגד מכבי פתח תקווה",
     "1:0 · הסתיים"
    ],
    [
     "מכבי נתניה נגד Hapoel Ramat Gan",
     "2:3 · הסתיים"
    ],
    [
     "Maccabi Ahi Nazeret נגד Hapoel Akko",
     "1:1 · הסתיים"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · עולם",
   "summary": "המשחקים החשובים בעולם כרגע, מהתחרויות המוכרות ביותר.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football",
   "image": null,
   "stats": [
    [
     "Shakhtar Donetsk נגד Kudrivka",
     "5:1 · הסתיים"
    ],
    [
     "Alashkert Martuni נגד Ararat Yerevan",
     "3:0 · הסתיים"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "קוז'וך לקראת הכוכב האדום: \"האמונה היא מעל הכל\"",
   "summary": "מאמן הפועל באר-שבע דיבר הערב לקראת המשחק הראשון מול אלופת סרביה (20:30) במסגרת הסיבוב השלישי במוקדמות ליגת האלופות: \"מבינים איזו יריבה עומדת מולנו\"",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelisoccer/article/bylrzvasfe",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2025/05/12/HyujCvhJ11xg/HyujCvhJ11xg_0_131_3000_1689_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "אשדוד גברה על מכבי יפו, ניצחון גם לבני יהודה",
   "summary": "הקבוצה מעיר הנמל ניצחה 0:2 את קבוצתו של איציק ברוך במסגרת המחזור השני של גביע הטוטו. הכתומים גברו 0:1 על מ.ס כפר קאסם משער של נחמני (53')",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/article/one529716",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/03/one1881123/one1881123_0_0_800_444_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "הפועל ת\"א סיכמה את העברתו של אווסו מהכוכב האדום בלגרד | פרסום ראשון",
   "summary": "ההעברה הגדולה בתולדות המועדון: האדומים יכמו את העברתו של שחקן הכנף הגנאי תמורת 2.5 מיליון אירו - והוא צפוי לחתום על חוזה לארבע עונות. מנכ\"ל הקבוצה גיא פרימור טס לבלגרד כדי לסגור את הפרטים האחרונים של העסקה",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelisoccer/article/bjebhsasfe",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/03/SkuXCBABfl/SkuXCBABfl_0_0_850_479_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "בר אותר ללא רוח חיים על קבר חברתו שנרצחה בנובה: \"נפשות תאומות\"",
   "summary": "\"גם החברים של הנרצחים הם נפגעי האסון\". שלושה ימים אחרי חתונת אחיו, בר בכור אסרף נמצא ללא רוח חיים ליד קברה של חברת ילדותו לירון ברדה שנרצחה ב-7 באוקטובר. \"גם שנתיים ו-9 חודשים אחרי, הכאב לא חלף\". שיתפו אמו ואחותו, שביקשו: \"תזכרו את בר לא בגלל סופו הטראגי - אלא…",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/s1ahmb0bge",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/03/HkqSNSCrGl/HkqSNSCrGl_115_75_443_250_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "הוצג \"מידע שגוי\"? גורם במועצת השלום: \"נתניהו עודכן בהכול כבר לפני שבועות\"",
   "summary": "סמוטריץ' וסטרוק טענו כי בקבינט הוצג להם \"מידע שגוי\" על התוכנית לעזה, אך במועצת השלום אומרים שמשרד רה\"מ והמל\"ל \"קיבלו את כל החומרים\". נסיגה לקו הצהוב? \"אלה רק בטונדות, זה משחקי מילים\". צה\"ל לא תקף בעזה זה כיממה. מועצת השלום הודיעה כי נסיגה מלאה תתרחש רק לאחר שח…",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/hkx2xu0hml",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/01/08/SklHns8pVWg/SklHns8pVWg_0_37_1280_721_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "הסטארטאפ שמצא חולשות חמורות ב-ChatGPT מגייס 125 מיליון דולר",
   "summary": "סוכני ה-AI כבר תופסים חלק מרכזי בתהליכי העבודה בארגונים, אבל יחד איתם מגיעות סכנות עמוקות בהרבה מסתם פרומפט בעייתי. הסטארטאפ הישראלי Zenity, שהתפרסם לא פעם בעקבות חשיפת חולשות במודלים הפופולריים בעולם, מציג גישה שונה לחלוטין להגנה על הארגון – ורושם אבן דרך בהת…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/zenity-mega-round/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "היזמת שמכרה חברה לסיילספורס גייסה 20 מיליון דולר ממארק בניוף ומייקל דל",
   "summary": "אפרת רפופורט כבר רשמה בעבר אקזיט של עשרות מיליוני דולרים לענקית ה-CRM סיילספורס, עסקה שהובילה אותה לנהל את מרכז הפיתוח הישראלי של החברה. עכשיו היא חוזרת לעולם היזמות, ומגייסת סבב Pre-Seed חריג בהיקפו, בהובלת לא אחר מאשר מארק בניוף, מייסד ומנכ\"ל Salesforce, וכן…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/june-ai-pre-seed/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "לינקדאין מציגה כפתור שיטפל בבעיה הכי גדולה שלה",
   "summary": "אם גם אתם שמתם לב שהפיד שלכם בלינקדאין התמלא בשנה האחרונה בפוסטים גנריים על \"למה פקקים באיילון הם בעצם הסדנה הכי טובה לפתרון צווארי בקבוק בפרודקט\" או \"איבדתי את האוזנייה של ה-AirPods. הנה מה שזה לימד אותי על החשיבות של Redundancy\"… אתם ממש לא לבד. הרשת החברתית…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/linkedin-ai-slop-button/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "האמן משה סעידי הלך לעולמו בגיל 89",
   "summary": "סעידי, שעלה לבדו מאיראן בגיל 13 ועסק באמנות קרמיקה במשך עשורים ארוכים, נפטר בשיבה טובה. לאורך השנים יצר קירות אמנות ויצירות המעטרים מבני ציבור ברחבי הארץ",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/bkvsag0hfx",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/03/SJ5Kf70Sfx/SJ5Kf70Sfx_0_62_1066_600_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "12 שעות של מוזיקה חיה: את מי תוכלו לראות בפסטיבל הרוק החדש בישראל?",
   "summary": "פסטיבל רעש לבן יעלה את מהדורתו הראשונה ב-9 באוקטובר בקיסריה, ויארח אמנים רבים בהם מוניקה סקס, פלד, הילה רוח, עלמה גוב, שייגעצ, מופע הארנבות של ד\"ר קספר ועוד. כרטיסים החל מ-239 שקלים",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/r1kqyzpbge",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/02/rJlqWZTBMe/rJlqWZTBMe_93_240_1207_680_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "בוסית קוגרית מנצלת מינית את העובד הצעיר שלה. נשמע פרובוקטיבי? לא ממש",
   "summary": "סרטו החדש של גרג אראקי, שהיה מהבמאים הנועזים של שנות ה-90, אומנם מהנה למדי, אבל מתקשה לשחזר את החדות והעוקץ שאפיינו את יצירותיו המוקדמות. אפילו הסצנות שעוסקות ביחסי השליטה והסקס שבין הגיבור לבוסית שלו נדמות יותר משועשעות מאשר בעלות אמירה של ממש",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/hkuyysnbmg",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/02/ByJlk22BGl/ByJlk22BGl_60_0_1441_811_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "לומדים",
   "learn": true,
   "title": "למה השמיים כחולים",
   "summary": "אור השמש מתפזר באטמוספרה, והצבע הכחול (גל קצר) מתפזר הכי הרבה — לכן אנחנו רואים אותו מכל הכיוונים.",
   "source": "תמצית ידע יומית · פיזיקה",
   "link": "",
   "stats": [
    [
     "תחום",
     "פיזיקה"
    ],
    [
     "עובדה בונוס",
     "דבש לא מתקלקל — נמצא דבש בן 3,000 שנה שעדיין אכיל."
    ]
   ]
  }
 ]
};