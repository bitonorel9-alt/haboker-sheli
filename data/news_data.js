window.NEWS_DATA = {
 "date": "יום ראשון, 16 באוגוסט 2026",
 "date_he": "16/08/2026",
 "updated": "05:18",
 "lead": {
  "section": "פוליטיקה",
  "eyebrow": "הכותרת של הבוקר",
  "title": "החזקים מתחזקים: הפערים בתקציב החינוך ברשויות המקומיות - וכמה משקיעים בילד שלכם",
  "summary": "חוקרות מאונ' בר אילן מראות כיצד תקציבי החינוך ברשויות המקומיות מעמיקים את הפערים בכל המערכת. בעוד יישובים מבוססים משקיעים במגמות ייחודיות ובמצוינות, רשויות חלשות נאלצות להסתפק בלשרוד. \"קודם שהתקרה לא דולפת, אחר כך שיש לילדים מה לאכול - ולא נשאר כסף\", מתאר מנהל…",
  "link": "https://www.ynet.co.il/news/article/yokra14865257",
  "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2014/05/12/5325213/5325213_0_23_1300_732_0_medium.jpg"
 },
 "articles": [
  {
   "section": "מזג אוויר",
   "title": "עתלית · 30.8° / 26.4°",
   "summary": "כרגע 27.6°, לחות 81%, רוח 9.7 קמ״ש. סיכוי משקעים 0%.",
   "source": "Open-Meteo · השירות המטאורולוגי",
   "link": "",
   "stats": [
    [
     "מקסימום",
     "30.8°"
    ],
    [
     "מינימום",
     "26.4°"
    ],
    [
     "לחות",
     "81%",
     81
    ],
    [
     "רוח",
     "9.7 קמ״ש"
    ],
    [
     "סיכוי גשם",
     "0%",
     0
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · ישראל",
   "summary": "עדכון חי ממשחקי הכדורגל בישראל - מתעדכן בכל איסוף.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football/israel",
   "image": null,
   "stats": [
    [
     "Hapoel Kfar Saba U19 נגד Bnei Yehuda U19",
     "0:1 · הסתיים"
    ],
    [
     "Maccabi Hertzliya U19 נגד Hapoel Tel Aviv U19",
     "0:1 · הסתיים"
    ],
    [
     "Hapoel Petah Tikva U19 נגד Kiryat Shmona U19",
     "2:1 · הסתיים"
    ],
    [
     "Hapoel Ra'anana U19 נגד Ironi Ramat Hasharon U19",
     "0:1 · הסתיים"
    ],
    [
     "Hapoel Rishon LeZion U19 נגד Hapoel Haifa U19",
     "0:2 · הסתיים"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "תוצאות בזמן אמת · עולם",
   "summary": "המשחקים החשובים בעולם כרגע, מהתחרויות המוכרות ביותר.",
   "source": "365Scores",
   "link": "https://www.365scores.com/he/football",
   "image": null,
   "stats": [
    [
     "LNZ Cherkasy נגד Chornomorets Odesa",
     "טרם התחיל"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "החמצת פנדל למסי בתבוסה של אינטר מיאמי, עוד לילה גדול לישראלים",
   "summary": "האלופה ו\"הפרעוש\" ספגו 4:1 כואב מנאשוויל, שהיה הפסדם השלישי ברציפות בכל המסגרות. בריבו כבש שער חמישי ברציפות, שלא הספיק לניצחון, עבדה הבקיע גם הוא ב-1:3 של שארלוט. הפסד לתורג'מן",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/worldsoccer/article/rkuwp3rugl",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/16/S1gYjt308ze/S1gYjt308ze_0_176_3000_1689_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "הפועל ת\"א תעלה בהרכב משני לחצי גמר גביע הטוטו: \"ניתן את כל תשומת הלב למשחק\"",
   "summary": "הקבוצה של אליניב ברדה עם הראש במשחק מול אטאלנטה במסגרת פלייאוף העלייה לקונפרנס-ליג ביום חמישי, אך המאמן לא מקל ראש בהתמודדות מול קריית שמונה הערב (20:00): \"חצי גמר הוא תמיד מעמד מרגש\"",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelisoccer/article/hjtcv3a8ge",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/11/BkerGggKLzx/BkerGggKLzx_0_192_3000_1689_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "כדורגל",
   "title": "סוכנו של רביבו לאלצ'ה: הגישו הצעה משופרת למכבי ת\"א | נחשף ב-ynet",
   "summary": "פאלי רמדאני סיפר כי הספרדים מוכנים לשלם 3 מיליון אירו, אך הצהובים מערימים קשיים ודורשים ויתור על כספים ואחוזים גבוהים ממכירה עתידית. במקביל: הנרי אדו, קרווין אנדרדה ויון ניקולאסקו בדרך החוצה כדי לפנות מקום לזרים חדשים",
   "source": "ynet - ספורט",
   "link": "https://www.ynet.co.il/sport/israelisoccer/article/ryl3ohrlmg",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/05/HyfpUv8e8Ml/HyfpUv8e8Ml_0_0_850_479_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - ספורט"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "מבית הכנסת התימני למסעדה החרדית: מסע עם המתמודדים בפריימריז בליכוד",
   "summary": "בחום הכבד של אוגוסט, בבתי כנסת ובמסעדות, מול קהל של כמה עשרות לעיתים, נלחמים מתמודדי הפריימריז בליכוד על עתידם הפוליטי | הנשק? קל: דבר כמה שיותר נגד מערכת המשפט, התקשורת והאקדמיה | טלי גוטליב מצטטת את ז'בוטינסקי, דודי אמסלם מודאג מהאדישות, ומירי רגב עוקצת ש\"צר…",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/yokra14864104",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/13/Hy5c9bjUMx/Hy5c9bjUMx_0_0_1920_1080_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "פוליטיקה",
   "title": "\"היחידה שלעולם לא תעזוב אותי\": העוזרת שהפכה לאישה הכי קרובה לטראמפ",
   "summary": "נטלי הארפ, העוזרת האישית של נשיא ארה\"ב, הפכה לאחד מהאנשים הקרובים אליו ביותר, והייתה אחת מ\"בני המזל\" שבצל האיום האיראני על חייו זכו לטוס איתו ב\"אייר פורס 1\" המאובטח. כתבי הניו יורק טיימס טוענים כי טראמפ אף הצהיר: \"היא היחידה שאוהבת אותי כמו המשפחה שלי\"",
   "source": "ynet - חדשות",
   "link": "https://www.ynet.co.il/news/article/yokra14865279",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/15/BkWcPNA8Mx/BkWcPNA8Mx_0_128_2248_1266_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - חדשות"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "3 אירועי טכנולוגיה חינמיים שבאמת תלמדו מהם משהו חדש",
   "summary": "ההייטק הישראלי לא עוצר: הנה המיטאפים, הכנסים והרצאות הטכנולוגיה המסקרנים שצפויים בקרוב. בפיזי או ברשת – אלו אירועי ההייטק המומלצים שאתם רוצים לשריין להם מקום ביומן. מארגנים אירוע או שמעתם על אירוע לקהילה הטכנולוגית בישראל? מוזמנים לעדכן אותנו כאן ולפרסם אותו ב…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/geek-events-1682026/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "עובדת טענה שהוחלפה ב-ChatGPT. המנכ\"ל: \"היה עדיף להחליף אותה ב-AI\"",
   "summary": "האם אנחנו עדים למקרה הראשון והמתוקשר שבו ה-AI מחליפה כותבים בתעשיית הגיימינג כמו שחששו וצפו רבים? כותבת בכירה טוענת שזה המקרה ומאשימה בפומבי את חברת הפיתוח שבה עבדה בהחלפתה במודל AI, אבל למנכ\"ל החברה יש תשובה חריפה, ישירה ולא קונבנציונלית, שמציתה עכשיו סערה חד…",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/employee-says-she-was-replaced-by-chatgpt-ceo-fires-back/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "AI",
   "title": "פיתוח ישראלי נעזר ב-AI כדי להוזיל את סל הקניות שלכם בסופר",
   "summary": "האם אפשר לנצח את התייקרות מוצרי המזון בעזרת מודלי שפה? מפתח ישראלי אחד חושב שכן. לצורך זה הוא הצליח לעשות סדר בבלאגן של רשתות השיווק, ובנה כלי שיודע להשוות בדיוק את סל הקניות האמיתי שלכם. אז לקחנו אותו לבדוק איפה באמת הכי זול. \"למנוע מהמודל להמציא מחיר\" הכירו …",
   "source": "גיקטיים",
   "link": "https://www.geektime.co.il/this-tool-will-help-you-proce-check-vefore-your-next-produce-run/",
   "image": null,
   "stats": [
    [
     "מקור",
     "גיקטיים"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "\"הייתי בטוח שאאבד עבודה ויתבעו אותי. ואז הלכתי לשחק כדורסל והעולם המשיך\"",
   "summary": "ג'רמי פוגל מאמין שפילוסופיה לא שייכת רק לאקדמיה, אלא יכולה לעזור לנו להבין את המציאות - גם כשהיא כאוטית. לרגל ספר הילדים החדש שלו, \"ספר ההפכים\", הוא מדבר על הסערה סביב הריאיון עם ח\"כ צבי סוכות (\"הם טובים בלייצר תוכן, אבל לא בלנהל מדינה\"), על האייטם בערוץ 14 שה…",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/r1vqzejigx",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/10/SyON1yP8Ge/SyON1yP8Ge_695_226_1054_594_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "המפה ללב של אבא / דנה ספקטור",
   "summary": "אם אבא שלי למד לאהוב את רן, כל אחד יכול לשנות את דעתו",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/yokra14861426",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2024/04/10/ryGYJ3JEgR/ryGYJ3JEgR_0_0_1920_1080_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "בידור",
   "title": "רותם שפי מצליחה להרגיז גם בלי שפיטה",
   "summary": "שפיטה אומנם עזרה לה לקנות בית, אבל רותם שפי כבר רצתה לשים  אותה בצד וגם 7 באוקטובר לא הגביר את החשק לקשקש בערבית. אז עכשיו היא מוציאה אלבום בכורה, לא מתחרטת שהכריזה על \"רילוקיישן\" בשביל לקדם שיר ובהחלט צועקת \"שאט אפ\" על כל מי שהתעצבן לראות אותה מיניקה. והגברת …",
   "source": "ynet - תרבות ובידור",
   "link": "https://www.ynet.co.il/entertainment/article/yokra14860170",
   "image": "https://ynet-pic1.yit.co.il/picserver6/crop_images/2026/08/10/Sk1lUVwIze/Sk1lUVwIze_0_0_1920_1080_0_medium.jpg",
   "stats": [
    [
     "מקור",
     "ynet - תרבות ובידור"
    ]
   ]
  },
  {
   "section": "לומדים",
   "learn": true,
   "title": "אפקט דנינג-קרוגר",
   "summary": "ככל שאדם יודע פחות בתחום, כך הוא נוטה להעריך את הידע שלו בו ביתר. מומחים אמיתיים דווקא מפחיתים בערך עצמם.",
   "source": "תמצית ידע יומית · מוח וקוגניציה",
   "link": "",
   "stats": [
    [
     "תחום",
     "מוח וקוגניציה"
    ],
    [
     "עובדה בונוס",
     "לתמנון יש שלושה לבבות ודם כחול."
    ]
   ]
  }
 ]
};